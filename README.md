...
void drawBitmap(int x, int y, int width, int height, const char &lowast;bitmap);

// Draw an XBM image with the given dimensions
void drawXbm(int x, int y, int width, int height, const char &lowast;xbm);
...
void setFont(const char &lowast;fontData);
```
...
void setFrameCallbacks(int frameCount, void (&lowast;frameCallbacks[])(SSD1306 &lowast;display, SSD1306UiState&lowast; state,int x, int y));
